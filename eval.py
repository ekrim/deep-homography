...
def read_and_convert(f, edges=None):
...
def prep_batch(f1, f2, edges, n_patches, big_patch, patch_size):
  ...
  for i in range(n_patches):
    delta_row = np.random.randint(0, max(img1.shape[0] - big_patch, 1))
    delta_col = np.random.randint(0, max(img1.shape[1] - big_patch, 1))
    delta_row = (img1.shape[0] - big_patch)//2
    delta_col = (img1.shape[1] - big_patch)//2
